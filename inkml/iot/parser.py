...
class IOTPaperParser(Parser):
    ...
    def __init__(self):
        self.__ink_parser: InkMLParser = InkMLParser()
    ...
    def parse(self, path_or_stream: Union[str, bytes, memoryview, BytesIO, Path]) -> InkModel:
        ...
        if inkml_element is not None:
            ...
            ink_model = self.__ink_parser.parse(inkml_string.encode('utf-8'))
            return ink_model
        raise FormatException("The IOT paper format contains no ink data")

    def parse_template(self, path_or_stream: Union[str, bytes, memoryview, BytesIO, Path]) -> Image:
        """
        ...
        -------
           image - `Image`
               Template image from the IOT paper format
        """
        if isinstance(path_or_stream, (str, pathlib.Path)):
            # It's a file path
            with open(path_or_stream, 'r') as inkml_file:
